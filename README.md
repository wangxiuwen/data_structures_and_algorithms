# 算法数据结构资料与题目记录

## 一般方法

    记忆细节，反复练习
    打破惯性，机器思维
    
## 练习重点

分治、回溯、递归、动态规划

## 核心思维

    升维
    空间换时间

## 资源

* [资源推荐](resources/recommand.md)
* [附件](resources/attachments.md)
* [刷题技巧](basics/skills.md)

## 基础理论

* [算法数据结构](basics/algorithm_and_data_structure.md)
* [时间/空间复杂度](basics/complexity.md)
* [数组 array](basics/array.md)
* [链表 linked list](basics/linked_list.md)
* [队列](basics/queue.md)
* [栈](basics/stack.md)
* [heap](basic/heap.md)
* [哈希表](basics/hash_table.md)
* [排序](basics/sort.md)
* [LRU Cache](basics/lru_cache.md)
* [递归](basics/recursion.md)
* [树](basics/tree.md)
* [trie 树](basics/trie.md)
* [并查集](basics/disjointSet.md)
* [深度优先搜索 & 广度优先搜索](basics/bst.md)
* [启发式搜索](basics/Astar.md)
* [分治 回溯](basics/devide_conquer.md)
* [贪心算法 Greedy](basics/greedy.md)
* [bloomfilter](basics/bloomfilter.md)
* [二分查找](basics/binary.md)
* [图](basics/grapth.md)
* [动态规划](basics/dynamic_programming.md)
* [高级搜索](basics/search.md)
* [位运算](basics/bitwise.md)
* [字符串](basics/strings.md)

## 高级理论

* [加密](basics/encryption.md)

## 题目链接

### 数组

* [1. 两数之和](leetcode/1.two-sum.md)    
* [66. 加一](leetcode/66.plus-one.md)    
* [11. 盛最多水的容器](leetcode/11.container-with-most-water.md)   
* [283. 移动零](leetcode/283.move-zeroes.md)   
* [70. 爬楼梯](leetcode/70.climbing-stairs.md)   
* [746. 使用最小花费爬楼梯](leetcode/746.min-cost-climbing-stairs.md)
* [15. 三数之和(高频老题)](leetcode/15.3sum.md)
* [18. 四数之和](leetcode/18.4sum.md)
* [16. 最接近的三数之和](leetcode/16.3sum-closest.md)
* [125. 验证回文串](leetcode/125.valid-palindrome.md)
* [238. 除自身以外数组的乘积](leetcode/238.product-of-array-except-self.md)

### 链表

* [206. 单链表反转](leetcode/206.reverse-linked-list.md)   
* [141. 链表中环的检测](leetcode/141.linked-list-cycle.md) 
* [142. 环形链表 II](leetcode/142.linked-list-cycle-ii)  
* [234. 回文链表](leetcode/234.palindrome-linked-list.md)  
* [19. 删除链表倒数第 n 个结点 ](leetcode/19.remove-nth-node-from-end-of-list.md)  
* [21. 两个有序的链表合并](leetcode/21.merge-two-sorted-lists.md)  
* [876. 求链表的中间结点](leetcode/876.middle-of-the-linked-list.md)  
* [24. 两两交换链表中的节点](leetcode/24.swap-nodes-in-pairs)  
* [25. K 个一组翻转链表](leetcode/25.reverse-nodes-in-k-group.md)  
* [26. 删除排序数组中的重复项](leetcode/26.remove-duplicates-from-sorted-array.md)   
* [189. 旋转数组](leetcode/189.rotate-array.md)  
* [88. 合并两个有序数组](leetcode/88.merge-sorted-array.md)  
* [61. 旋转链表](leetcode/61.rotate-list.md) 
* [237. 删除链表中的节点](leetcode/237.delete-node-in-a-linked-list.md)
* [148. 排序链表](leetcode/148.sort-list.md)
* [面试题 02.07. 链表相交](leetcode/intersection-of-two-linked-lists-lcci.md)


### 数组/链表

* [146. LRU缓存机制](leetcode/146.lru-cache.md)  

### 栈

* [20. 有效的括号](leetcode/20.valid-parentheses.md)   
* [42. 接雨水](leetcode/42.trapping-rain-water.md)   
* [155. 最小栈](leetcode/155.min-stack.md)   
* [84. 柱状图中最大的矩形](leetcode/84.largest-rectangle-in-histogram.md)  
* [239. 滑动窗口最大值(用双指针再做一遍)](leetcode/239.sliding-window-maximum.md)   
* [641. 设计循环双端队列](leetcode/641.design-circular-deque.md)    
* [844. 比较含退格的字符串](leetcode/844.backspace-string-compare.md)
* [224. 基本计算器](leetcode/224.basic-calculator.md)
* [682. 棒球比赛](leetcode/682.baseball-game.md)
* [496. 下一个更大元素 I](leetcode/496.next-greater-element-i.md)
* [739. 每日温度](leetcode/739.daily-temperatures.md)

### 哈希表

* [242. 有效的字母异位词](leetcode/242.valid-anagram.md)  
* [49. 字母异位词分组](leetcode/49.group-anagrams.md)  
* [1. 两数之和](leetcode/1.two-sum.md)   

### 队列

* [933. 最近的请求次数](leetcode/933.number-of-recent-calls.md)  

### 队列/栈

* [232. 用栈实现队列](leetcode/232.implement-queue-using-stacks.md)  
* [225. 用队列实现栈](leetcode/225.implement-stack-using-queues.md)  

### 堆

* [215. 数组中的第K个最大元素](leetcode/215.kth-largest-element-in-an-array.md)
* [703. 数据流中的第K大元素(小顶堆)](leetcode/703.kth-largest-element-in-a-stream.md)
* [239. 滑动窗口最大值(栈)](leetcode/239.sliding-window-maximum.md)

### 递归

* [70. 爬楼梯(用换零钱的方法做一遍)](leetcode/70.climbing-stairs.md)    
* [22. 括号生成](leetcode/22.generate-parentheses.md)   
* [77. 组合](leetcode/77.combinations.md)  
* [46. 全排列](leetcode/46.permutations.md)   
* [47. 全排列 II](leetcode/47.permutations-ii.md)   

### 树/递归

* [98. 验证二叉搜索树(中序遍历)](leetcode/98.validate-binary-search-tree.md)  
* [104. 二叉树的最大深度](leetcode/104.maximum-depth-of-binary-tree.md)   
* [226. 翻转二叉树](leetcode/226.invert-binary-tree.md)  
* [111. 二叉树的最小深度](leetcode/111.minimum-depth-of-binary-tree)  
* [297. 二叉树的序列化与反序列化](leetcode/297.serialize-and-deserialize-binary-tree.md)  
* [236. 二叉树的最近公共祖先](leetcode/236.lowest-common-ancestor-of-a-binary-tree.md)  
* [235. 二叉搜索树的最近公共祖先](leetcode/235.lowest-common-ancestor-of-a-binary-search-tree.md)
* [105. 从前序与中序遍历序列构造二叉树](leetcode/105.construct-binary-tree-from-preorder-and-inorder-traversal)  
* [94. 二叉树的中序遍历(用手动维护栈的方式遍历和莫里斯遍历做一遍)](leetcode/94.binary-tree-inorder-traversal.md)  
* [144. 二叉树的前序遍历](leetcode/144.binary-tree-preorder-traversal.md)  
* [590. N叉树的后序遍历](leetcode/590.n-ary-tree-postorder-traversal.md)  
* [589. N叉树的前序遍历](leetcode/589.n-ary-tree-preorder-traversal.md)  
* [429. N叉树的层序遍历](leetcode/429.n-ary-tree-level-order-traversal.md)    
* [124. 二叉树中的最大路径和](leetcode/124.binary-tree-maximum-path-sum.md)
* [99. 恢复二叉搜索树](leetcode/99.recover-binary-search-tree.md)
* [230. 二叉搜索树中第K小的元素](leetcode/230.kth-smallest-element-in-a-bst.md)

### 分治

* [50. Pow(x, n)](leetcode/50.powx-n.md) 
* [78. 子集](leetcode/78.subsets.md) 
* [17. 电话号码的字母组合](leetcode/17.letter-combinations-of-a-phone-number.md)  
* [51. N皇后](leetcode/51.n-queens.md)  
* [169. 多数元素(可以用哈希表做数学统计，也可以用分治, 用分治再做一遍)](leetcode/169.majority-element.md)

### BFS/DFS

* [102. 二叉树的层次遍历 (高频题排名前三)](leetcode/102.binary-tree-level-order-traversal.md)  
* [433. 最小基因变化](leetcode/433.minimum-genetic-mutation.md)  
* [22. 括号生成(DFS，BFS 也可)](leetcode/22.generate-parentheses.md)   
* [200. 岛屿数量(沉岛法)](leetcode/200.number-of-islands.md)  
* [515. 在每个树行中找最大值](leetcode/515.find-largest-value-in-each-tree-row.md)  
* [127. 单词接龙](leetcode/127.word-ladder.md) 
* [126. 单词接龙 II](leetcode/126.word-ladder-ii.md)
* [529. 扫雷游戏](leetcode/529.minesweeper.md)

## 双向 BFS/DFS

* [127. 单词接龙](leetcode/127.word-ladder.md) 
* [433. 最小基因变化](leetcode/433.minimum-genetic-mutation.md)  

## A* Search

* [1091. 二进制矩阵中的最短路径](leetcode/1091.shortest-path-in-binary-matrix.md)
* [773. 滑动谜题](leetcode/773.sliding-puzzle.md) 
* [37. 解数独](leetcode/37.sudoku-solver.md)

### 贪心算法

* [455. 分发饼干(贪心)](leetcode/455.assign-cookies.md)   
* [322. 零钱兑换(贪心、分治、动态规划)](leetcode/322.coin-change.md) 
* [121. 买卖股票的最佳时机(贪心、分治、动态规划)](leetcode/121.best-time-to-buy-and-sell-stock.md)
* [122. 买卖股票的最佳时机 II](leetcode/122.best-time-to-buy-and-sell-stock-ii.md)
* [55. 跳跃游戏](leetcode/55.jump-game.md)  
* [](leetcode/jump-game-ii.md)  
* [](leetcode/lemonade-change.md)  
* [](leetcode/walking-robot-simulation.md)  
  

### 二分查找

使用二分查找，寻找一个半有序数组 [4, 5, 6, 7, 0, 1, 2] 中间无序的地方

* [69. x 的平方根(二分查找)](leetcode/69.sqrtx.md)
* [367. 有效的完全平方数(动态规划，位运算也可)](leetcode/367.valid-perfect-square.md) 
* [33. 搜索旋转排序数组](leetcode/33.search-in-rotated-sorted-array.md)  
* [74. 搜索二维矩阵](leetcode/74.search-a-2d-matrix.md) 
* [153. 寻找旋转排序数组中的最小值](leetcode/153.find-minimum-in-rotated-sorted-array.md)

### Trie 树

* [208. 实现 Trie (前缀树)](leetcode/208.implement-trie-prefix-tree.md)
* [79. 单词搜索](leetcode/79.word-search.md)
* [212. 单词搜索 II(分析用 Tire 树方式实现的时间复杂度)](leetcode/212.word-search-ii.md)

## 并查集

* [200. 岛屿数量(染色(沉岛法，dfs/bfs)/并查集)](leetcode/200.number-of-islands.md)
* [547. 朋友圈](leetcode/547.friend-circles.md)
* [](leetcode/surrounded-regions.md)

### 动态规划

#### 初级一维

* [70. 爬楼梯](leetcode/70.climbing-stairs.md)  
* [1143.最长公共子序列(贪心、分治、动态规划)](leetcode/1143.longest-common-subsequence.md)  
* [120. 三角形最小路径和](leetcode/120.triangle.md)  
* [53. 最大子序和(理解一下分治的做法)](leetcode/53.maximum-subarray)  
* [152. 乘积最大子数组(贪心，分治，动态规划)](leetcode/152.maximum-product-subarray.md)
* [121. 买卖股票的最佳时机](leetcode/121.best-time-to-buy-and-sell-stock.md)
* [122. 买卖股票的最佳时机 II](leetcode/122.best-time-to-buy-and-sell-stock-ii.md)
* [123. 买卖股票的最佳时机 III](leetcode/123.best-time-to-buy-and-sell-stock-iii.md)
* [309. 最佳买卖股票时机含冷冻期](leetcode/309.best-time-to-buy-and-sell-stock-with-cooldown.md)
* [188. 买卖股票的最佳时机 IV](leetcode/188.best-time-to-buy-and-sell-stock-iv.md)
* [714. 买卖股票的最佳时机含手续费](leetcode/714.best-time-to-buy-and-sell-stock-with-transaction-fee.md)
* [](leetcode/longest-valid-parentheses.md)
* [](leetcode/minimum-path-sum.md)
* [](leetcode/decode-ways)
* [](leetcode/maximal-square.md)
* [](leetcode/max-sum-of-rectangle-no-larger-than-k.md)
* [](leetcode/frog-jump.md)
* [](leetcode/split-array-largest-sum.md)
* [](leetcode/student-attendance-record-ii.md)
* [](leetcode/task-scheduler.md)
* [](leetcode/palindromic-substrings.md)
* [](leetcode/minimum-window-substring.md)
* [](leetcode/burst-balloons.md)

#### 高级多维

* [198. 打家劫舍](leetcode/198.house-robber.md)
* [](leetcode/house-robber-ii.md)
* [72. 编辑距离(重点)](leetcode/72.edit-distance.md) 
* [](leetcode/perfect-squares.md)
* [](leetcode/jump-game.md)
* [](leetcode/jump-game-ii.md)
* [62.不同路径](leetcode/62.unique-paths.md)  
* [63.不同路径 II(要求写出状态方程)](leetcode/63.unique-paths-ii.md)  
* [](leetcode/unique-paths-iii.md)
* [322. 零钱兑换(贪心、分治、动态规划)](leetcode/322.coin-change.md)
* [](leetcode/coin-change-2.md)
* [300. 最长上升子序列](leetcode/300.longest-increasing-subsequence.md)
* [](leetcode/maximal-rectangle.md)
* [115. 不同的子序列](leetcode/115.distinct-subsequences.md)
* [](leetcode/race-car.md)

### 搜索

* [70. 爬楼梯(动态规划， 递归去重也可，转换零钱兑换问题)](leetcode/70.climbing-stairs.md)

### 剪枝

* [22. 括号生成(动态规划，剪枝) ](leetcode/22.generate-parentheses.md) 
* [51(n皇后)](leetcode/51.n-queens.md)
* [36. 有效的数独](leetcode/36.valid-sudoku.md) 
* [37. 解数独](leetcode/37.sudoku-solver.md)

### 启发式搜索(A*)

* [1091. 二进制矩阵中的最短路径(dp, bfs, A*)](leetcode/1091.shortest-path-in-binary-matrix.md)

### 位运算

* [136. 只出现一次的数字](leetcode/136.single-number.md)
* [190. 颠倒二进制位](leetcode/190.reverse-bits.md)
* [191. 位1的个数](leetcode/191.number-of-1-bits.md)  
* [231. 2的幂](leetcode/231.power-of-two.md)  
* [338. 比特位计数](leetcode/338.counting-bits.md)  
* [51. N皇后(n皇后终极解法)](leetcode/51.n-queens.md) 
* [52. N皇后 II](leetcode/52.n-queens-ii.md)

### 排序

* [242. 有效的字母异位词](leetcode/242.valid-anagram.md)
* [56. 合并区间](leetcode/56.merge-intervals.md)
* [493. 翻转对(树状数组/归并排序)](leetcode/493.reverse-pairs.md)  
* [](leetcode/relative-sort-array.md)  
* [](leetcode/design-a-leaderboard.md)  

### 字符串

两个字符串变换问题，一般要转成一个二维数组，两个字符串分别为字符串的行和列

基础问题

* [](leetcode/to-lower-case.md)
* [](leetcode/length-of-last-word.md)
* [](leetcode/jewels-and-stones.md)
* [387. 字符串中的第一个唯一字符](leetcode/387.first-unique-character-in-a-string.md)
* [8. 字符串转换整数 (atoi)](leetcode/8.string-to-integer-atoi.md)

字符串操作问题

* [14. 最长公共前缀](leetcode/14.longest-common-prefix.md)
* [344. 反转字符串](leetcode/344.reverse-string.md)
* [](leetcode/reverse-string-ii.md)
* [151. 翻转字符串里的单词](leetcode/151.reverse-words-in-a-string.md)
* [](leetcode/reverse-words-in-a-string-iii.md)
* [](leetcode/reverse-only-letters.md)

Anagram异位词问题

* [](leetcode/valid-anagram.md)
* [](leetcode/group-anagrams.md)
* [438. 找到字符串中所有字母异位词](leetcode/438.find-all-anagrams-in-a-string.md)

Palindrome 回文串问题

* [](leetcode/valid-palindrome.md)
* [](leetcode/valid-palindrome-ii.md)
* [5. 最长回文子串](leetcode/5.longest-palindromic-substring.md)

最长子串、子序列问题

* [](leetcode/longest-common-subsequence.md)
* [72. 编辑距离(重点)](leetcode/72.edit-distance.md)
* [](leetcode/longest-palindromic-substring.md)

字符串 +DP 问题

* [10. 正则表达式匹配](leetcode/10.regular-expression-matching.md)
* [](leetcode/wildcard-matching.md)
* [115. 不同的子序列](leetcode/115.distinct-subsequences.md)

其它

* [](leetcode/first-unique-character-in-a-string.md)
* [](leetcode/reverse-string-ii.md)
* [](leetcode/reverse-words-in-a-string.md)
* [](leetcode/reverse-words-in-a-string-iii.md)
* [](leetcode/reverse-only-letters.md)
* [](leetcode/find-all-anagrams-in-a-string.md)
* [](leetcode/longest-palindromic-substring.md) 
* [](leetcode/isomorphic-strings.md)
* [](leetcode/valid-palindrome-ii.md)
* [](leetcode/wildcard-matching.md)
* [](leetcode/longest-valid-parentheses.md)

## leetcode 解法

* [1. 两数之和](leetcode/1.two-sum.md)
* [5. 最长回文子串](leetcode/5.longest-palindromic-substring.md)
* [5. 最长回文子串](leetcode/5.longest-palindromic-substring.md)
* [7. 整数反转](leetcode/7.reverse-integer.md)
* [8. 字符串转换整数 (atoi)](leetcode/8.string-to-integer-atoi.md)
* [10. 正则表达式匹配(贪心、分治、动态规划)](leetcode/10.regular-expression-matching.md)
* [11. 盛最多水的容器](leetcode/11.container-with-most-water.md)
* [14. 最长公共前缀](leetcode/14.longest-common-prefix.md)
* [15. 三数之和](leetcode/15.3sum.md)
* [16. 最接近的三数之和](leetcode/16.3sum-closest.md)
* [17. 电话号码的字母组合](leetcode/17.letter-combinations-of-a-phone-number.md)
* [18. 四数之和](leetcode/18.4sum.md)
* [19. 删除链表的倒数第N个节点](leetcode/19.remove-nth-node-from-end-of-list.md)
* [20. 有效的括号(栈)](leetcode/20.valid-parentheses.md)
* [21. 合并两个有序链表](leetcode/21.merge-two-sorted-lists.md)
* [22. 括号生成](leetcode/22.generate-parentheses.md)
* [23. 合并K个排序链表](leetcode/23.merge-k-sorted-lists.md)
* [24. 两两交换链表中的节点](leetcode/24.swap-nodes-in-pairs.md)
* [25. K 个一组翻转链表](leetcode/25.reverse-nodes-in-k-group.md)
* [26. 删除排序数组中的重复项](leetcode/26.remove-duplicates-from-sorted-array.md)
* [32. 最长有效的括号(栈)](leetcode/longest-valid-parentheses.md)
* [33. 搜索旋转排序数组](leetcode/33.search-in-rotated-sorted-array.md)
* [34. 在排序数组中查找元素的第一个和最后一个位置](leetcode/34.find-first-and-last-position-of-element-in-sorted-array.md)
* [36. 有效的数独](leetcode/36.valid-sudoku.md)
* [37. 解数独](leetcode/37.sudoku-solver.md)
* [41. 缺失的第一个正数(数组)](leetcode/41.first-missing-positive.md)
* [43. 字符串相乘](leetcode/43.multiply-strings.md)
* [42. 接雨水](leetcode/42.trapping-rain-water.md)
* [46. 全排列](leetcode/46.permutations.md)
* [47. 全排列 II](leetcode/47.permutations-ii.md)
* [49. 字母异位词分组](leetcode/49.group-anagrams.md)  
* [50. Pow(x, n)](leetcode/50.powx-n.md)
* [51. n 皇后](leetcode/51.n-queens.md)
* [52. N皇后 II](leetcode/52.n-queens-ii.md)
* [53. 最大子序和](leetcode/53.maximum-subarray.md)
* [55. 跳跃游戏](leetcode/55.jump-game.md)
* [56. 合并区间](leetcode/56.merge-intervals.md)
* [61. 旋转链表](leetcode/61.rotate-list.md) 
* [62. 不同路径](leetcode/62.unique-paths.md)
* [63. 不同路径 II](leetcode/63.unique-paths-ii.md)  
* [66. 加一](leetcode/66.plus-one.md)
* [69. x 的平方根(二分查找)](leetcode/69.sqrtx.md)
* [70. 爬楼梯](leetcode/70.climbing-stairs.md)
* [72. 编辑距离(重点)](leetcode/72.edit-distance.md) 
* [74. 搜索二维矩阵](leetcode/74.search-a-2d-matrix.md)
* [75. 颜色分类](leetcode/75.sort-colors.md)
* [78. 子集](leetcode/78.subsets.md)
* [79. 单词搜索](leetcode/79.word-search.md)
* [84. 柱状图中最大的矩形](leetcode/84.largest-rectangle-in-histogram.md)
* [88. 合并两个有序数组](leetcode/88.merge-sorted-array.md)
* [94. 二叉树的中序遍历](leetcode/94.binary-tree-inorder-traversal.md)
* [98. 验证二叉搜索树](leetcode/98.validate-binary-search-tree.md)
* [99. 恢复二叉搜索树](leetcode/99.recover-binary-search-tree.md)
* [102. 二叉树的层序遍历](leetcode/102.binary-tree-level-order-traversal.md)
* [104. 二叉树的最大深度](leetcode/104.maximum-depth-of-binary-tree.md)
* [105. 从前序与中序遍历序列构造二叉树](leetcode/105.construct-binary-tree-from-preorder-and-inorder-traversal.md)
* [111. 二叉树的最小深度](leetcode/111.minimum-depth-of-binary-tree.md)
* [115. 不同的子序列](leetcode/115.distinct-subsequences.md)
* [120. 三角形最小路径和(贪心、分治、动态规划)](leetcode/120.triangle.md)
* [121. 买卖股票的最佳时机(贪心、分治、动态规划)](leetcode/121.best-time-to-buy-and-sell-stock.md)
* [122. 买卖股票的最佳时机 II](leetcode/122.best-time-to-buy-and-sell-stock-ii.md)
* [123. 买卖股票的最佳时机 III](leetcode/123.best-time-to-buy-and-sell-stock-iii.md)
* [124. 二叉树中的最大路径和](leetcode/124.binary-tree-maximum-path-sum.md)
* [125. 验证回文串](leetcode/125.valid-palindrome.md)
* [127. 单词接龙](leetcode/127.word-ladder.md)  
* [136. 只出现一次的数字](leetcode/136.single-number.md)
* [141. 环形链表](leetcode/141.linked-list-cycle.md)
* [142. 环形链表 II](leetcode/142.linked-list-cycle-ii.md)
* [144. 二叉树的前序遍历](leetcode/144.binary-tree-preorder-traversal.md)
* [146. LRU缓存机制](leetcode/146.lru-cache.md)
* [148. 排序链表](leetcode/148.sort-list.md)
* [151. 翻转字符串里的单词](leetcode/151.reverse-words-in-a-string.md)
* [152. 乘积最大子数组](leetcode/152.maximum-product-subarray.md)
* [155. 最小栈](leetcode/155.min-stack.md)
* [164. 最大间距](leetcode/164.maximum-gap.md)
* [169. 多数元素](leetcode/169.majority-element.md)
* [179. 最大数](leetcode/179.largest-number.md)
* [188. 买卖股票的最佳时机 IV](leetcode/188.best-time-to-buy-and-sell-stock-iv.md)
* [189. 旋转数组](leetcode/189.rotate-array.md)
* [190. 颠倒二进制位](leetcode/190.reverse-bits.md)
* [191. 位1的个数](leetcode/191.number-of-1-bits.md)  
* [198. 打家劫舍](leetcode/198.house-robber.md)
* [200. 岛屿数量](leetcode/200.number-of-islands.md)
* [206. 反转列表](leetcode/206.reverse-linked-list.md)
* [208. 实现 Trie (前缀树)](leetcode/208.implement-trie-prefix-tree.md)
* [212. 单词搜索 II](leetcode/212.word-search-ii.md)
* [215. 数组中的第K个最大元素](leetcode/215.kth-largest-element-in-an-array.md)
* [225. 用队列实现栈](leetcode/225.implement-stack-using-queues.md)  
* [226. 翻转二叉树](leetcode/226.invert-binary-tree.md)
* [230. 二叉搜索树中第K小的元素](leetcode/230.kth-smallest-element-in-a-bst.md)
* [231. 2的幂](leetcode/231.power-of-two.md)  
* [232. 用栈实现队列](leetcode/232.implement-queue-using-stacks.md)
* [234. 回文链表](leetcode/234.palindrome-linked-list.md)
* [235. 二叉搜索树的最近公共祖先](leetcode/235.lowest-common-ancestor-of-a-binary-search-tree.md)
* [236. 二叉树的最近公共祖先](leetcode/236.lowest-common-ancestor-of-a-binary-tree.md)
* [237. 删除链表中的节点](leetcode/237.delete-node-in-a-linked-list.md)
* [238. 除自身以外数组的乘积](leetcode/238.product-of-array-except-self.md)
* [239. 滑动窗口最大值(栈)](leetcode/239.sliding-window-maximum.md)
* [242. 有效的字母异位词](leetcode/242.valid-anagram.md)
* [283. 移动零](leetcode/283.move-zeroes.md)
* [292. Nim 游戏](leetcode/292.nim-game.md)
* [297. 二叉树的序列化与反序列化](leetcode/297.serialize-and-deserialize-binary-tree.md)
* [300. 最长上升子序列](leetcode/300.longest-increasing-subsequence.md)
* [309. 最佳买卖股票时机含冷冻期](leetcode/309.best-time-to-buy-and-sell-stock-with-cooldown.md)
* [322. 零钱兑换(贪心、分治、动态规划)](leetcode/322.coin-change.md)
* [338. 比特位计数](leetcode/338.counting-bits.md)  
* [344. 反转字符串](leetcode/344.reverse-string.md)
* [367. 有效的完全平方数(动态规划，位运算也可)](leetcode/367.valid-perfect-square.md)
* [387. 字符串中的第一个唯一字符](leetcode/387.first-unique-character-in-a-string.md)
* [388. 文件的最长绝对路径](leetcode/388.longest-absolute-file-path.md)
* [429. N叉树的层序遍历](leetcode/429.n-ary-tree-level-order-traversal.md)
* [433. 最小基因变化](leetcode/433.minimum-genetic-mutation.md)  
* [438. 找到字符串中所有字母异位词](leetcode/438.find-all-anagrams-in-a-string.md)
* [449. 序列化和反序列化二叉搜索树](leetcode/449.serialize-and-deserialize-bst.md)
* [455. 分发饼干(贪心)](leetcode/455.assign-cookies.md)
* [493. 翻转对(树状数组/归并排序)](leetcode/493.reverse-pairs.md)  
* [515. 在每个树行中找最大值](leetcode/515.find-largest-value-in-each-tree-row.md)
* [547. 朋友圈](leetcode/547.friend-circles.md)
* [557. 反转字符串中的单词 III](leetcode/557.reverse-words-in-a-string-iii.md)
* [589. N叉树的前序遍历](leetcode/589.n-ary-tree-preorder-traversal.md)
* [590. N叉树的后序遍历](leetcode/590.n-ary-tree-postorder-traversal.md)
* [703. 数据流中的第K大元素](leetcode/703.kth-largest-element-in-a-stream.md)
* [714. 买卖股票的最佳时机含手续费](leetcode/714.best-time-to-buy-and-sell-stock-with-transaction-fee.md)
* [739. 每日温度](leetcode/739.daily-temperatures.md)
* [746. 使用最小花费爬楼梯](leetcode/746.min-cost-climbing-stairs.md)
* [773. 滑动谜题](leetcode/773.sliding-puzzle.md)
* [876. 链表的中间结点](leetcode/876.middle-of-the-linked-list.md)
* [933. 最近的请求次数(队列)](leetcode/933.number-of-recent-calls.md)
* [1091. 二进制矩阵中的最短路径(dp, bfs, A*)](leetcode/1091.shortest-path-in-binary-matrix.md)
* [1137. 第 N 个泰波那契数](leetcode/1137.n-th-tribonacci-number.md)
* [1143. 最长公共子序列](leetcode/1143.longest-common-subsequence.md)


## 程序员面试经典 解法

* [面试题 02.07. 链表相交](golden/intersection-of-two-linked-lists-lcci.md)

#### 未整理

64. 最小路径和（贪心、分治、动态规划）
632. 最小区间	
150. 逆波兰表达式求值(栈)

8. 字符串转换整数 (atoi)（字符串）
112. 路径总和
451. 根据字符出现频率排序	

966. 元音拼写检查器	
705. 设计哈希集合
347. 前 K 个高频元素

295. 数据流的中位数	
421. 数组中两个数的最大异或值
402. 移掉K位数字

621. 任务调度器	
435. 无重叠区间	
93. 复原IP地址

1206. 设计跳表
207. 课程表
787. K 站中转内最便宜的航班

743. 网络延迟时间
28. 实现 strStr()
30. 串联所有单词的子串
